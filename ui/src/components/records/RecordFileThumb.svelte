<script>
    ...
    $: originalUrl = !isLoadingToken ? Pocketbase.files.getUrl(record, filename, { token }) : "";

    $: thumbUrl = !isLoadingToken
        ? Pocketbase.files.getUrl(record, filename, { thumb: "100x100", token: token })
        : "";
    ...
    async function loadFileToken() {
        ...
        try {
            token = await Pocketbase.getAdminFileToken(record.collectionId);
        } catch (err) {
